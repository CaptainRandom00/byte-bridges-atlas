...
function generateString(D, dataPathHint) {
  ...
  function buildOverview(){
    ...
    if (devlogList.length) {
      const devlog = devlogList.map(e=>{
        const hasSecTag = Array.isArray(e.tags) && e.tags.includes('security');
        return `
        <div class="log-entry${hasSecTag?' log-security':''}"><div class="log-date">${esc(e.date)}</div>
          <div class="log-body"><div class="log-title">${hasSecTag?'<span class="log-shield" title="Security-tagged entry">🛡</span> ':''}${esc(e.title)} ${(e.tags||[]).map(t=>`<span class="log-tag${t==='security'?' log-tag-sec':''}">${esc(t)}</span>`).join('')}</div>
  ...
  function buildSprint(s){
    ...
    const rows = (s.tasks||[]).map(t=>{
      ...
      return `
      <div class="task${sec?' task-with-sec':''}">
        <div class="task-top">
